class UserProjectsController < ApplicationController
  ...
  def create
    if params[:project_id].present?
      @user_project = UserProject.new(project_id: params[:project_id], user: current_user)
    else
      project = Project.find_by_search(params[:project_name])
      if project
        @user_project = UserProject.new(user: current_user, project: project)
      else
        project = Project.new_from_lookup(params[:project_name])
        if project.save
          @user_project = UserProject.new(user: current_user, project: project)
        else
          @user_project = nil
          flash[:error] = "Project not saved. Please try again.."
        end
      end
    end
